...
# Dropping highly similar independent features
x.drop(['High', 'Low', 'Close','WCP', 'AP', 'MP', 'TP'],axis=1,inplace=True)
...
# Dropping the features with very little information gain
x.drop(['Open','Volume', 'MA', 'BOP','MOM', 'ROC', 'RSI'],axis=1,inplace=True)
...
for train_index, test_index in kf.split(x):
    ...
    precision_set.append(format(precision_score(y_test, prediction, average='macro'),".4f"))
    recall_set.append(format(recall_score(y_test, prediction, average='macro'),".4f"))
    f1_score_set.append(format(f1_score(y_test, prediction, average='micro'), ".4f"))
    ...
    t = Table()
    t['classification'] = ['KNN ','DT ','RF', 'SGD', 'AdaBoost', 'MLP', 'SVM', 'GBC', 'XGB', 'ETC']
